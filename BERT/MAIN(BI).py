...
from BiLSTM.model.BiLSTM import BI
...
model = BI(input_dim=768, hidden_dim=768, num_layers=2,block_nums=5,output_dim=2)
...
for epoch in range(epochs):
    ...
    for batch in progress_bar:
        ...
        with torch.no_grad():
            outputs = bert.teacher_bert.embeddings(input_ids)


        actual_lengths = attention_mask.sum(dim=1)


        outputs = model(x=outputs,lengths=actual_lengths)
